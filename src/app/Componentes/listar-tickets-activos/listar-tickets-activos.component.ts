...
export class ListarTicketsActivosComponent implements OnInit {
  ...
  listarTickets() {
    this.estacionamiento.listarTickets()
      .subscribe(res => {
        ...
        this.toast.error(err,"error al listar vehículos:");
      });
  }
  ...
  registrarSalidaTicket(licensePlate) {
    this.estacionamiento.registrarSalida(licensePlate).subscribe(
      res => {
        if(res){
          this.toast.success("se registró la salida del vehiculo " + licensePlate + ", con un costo de" + res['value'],"Salida del estacionamiento:");
          this.listarTickets();
        }else{
          this.toast.error("ocurrió un error al registrar salida");
        }
      }, err => {
